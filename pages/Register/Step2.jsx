...
const step1 = observer(() => {
    ...
    useFocusEffect(
        useCallback(
            () => {
                setTimeout(()=>{
                   // console.log('set')
                    userStore.setRegisterProgress(0.66)
                },50)
            },[],
        )
    )
    ...
    const choosePic = (pic)=>{
        userStore.setSubmitInfo('backgroundImage',pic)
        ...
        <View style={{backgroundColor:"#FFFFFF",paddingTop:0,width:screenSize.width,height:screenSize.height}}>
            <TouchableWithoutFeedback onPress={()=>Keyboard.dismiss()} style={{padding:20,width:screenSize.width}}>
                ...
                <View style={{flexDirection:"row",paddingLeft:10}}>
                    ...
                    <Text style={{lineHeight:50,paddingLeft:20,fontSize:18,fontWeight:"500"}} >背景圖片</Text>
            ...
            <View style={{padding:20,marginBottom:50}}>
                <View onTouchStart={()=>setIsOpen(()=>true)} style={{justifyContent:"center",alignItems:'center',width:screenSize.width-40,height:400,borderColor:"#CDCDCD" ,borderWidth:1,borderRadius:20}}>
                    {
                        userStore.submitInfo.backgroundImage===''?
                        <Image source={require('../../assets/icon.png')} style={{width:screenSize.width-40,height:400,borderRadius:20,position:'absolute',top:0}}  />
                        :
                        <Image source={{uri:userStore.submitInfo.backgroundImage}} style={{width:screenSize.width-40,height:400,borderRadius:20,position:'absolute',top:0}}  />
                    }
                    {
                        userStore.submitInfo.backgroundImage===''
                        &&
            ...
            <View style={{justifyContent:"center",alignItems:"center",width:screenSize.width}}>
                <View style={{backgroundColor:"#28C1FD",width:250,height:50,borderRadius:20,justifyContent:'center',alignItems:'center'}}>
                    <TouchableHighlight activeOpacity={0.7}  underlayColor="#FFFFFF" onPress={()=>navigation.navigate("step3")} style={{borderRadius:20}} >
