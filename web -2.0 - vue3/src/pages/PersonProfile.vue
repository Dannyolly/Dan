...
import PH from '@/pages/Profile/ProfileHeader.vue'
...
const loadData = async ()=>{
    ...
    let userId = isPersonProfile.value?
        LocalStorageManager.getLocalStorageInfo('userInfo')?.id
        :
        //@ts-ignore
        userInfo.value.user_id? userInfo.value.user_id: userInfo.value?.id
...
const BottomEl  = ()=>{
    return (
        <>
        {
            ...
            <EmptyVue   desc='NO POST ANYMORE' imageSize={300} />
        }
        </>
    )
}
...
const load = async ()=>{
  ...
  userInfo.value = info.userInfo || info;
