...
import { CollectPost, FormattedPost, Post } from '@/types/post';
...
import { InformType, PostNotification } from '@/types/Notification';
import { useUserStore } from '@/store/user';
interface PostProps {
    info : FormattedPost & PostNotification & CollectPost
    deleteData?:()=>void,
    uuid?:string,
    itemProps?:any
    /**
...
const likeAction = async (e:MouseEvent)=>{
    ...
    if(!isLike.value){
        ...
        if(userId !== toId){
            sendNotification(userId,toId,InformType.LIKE,post.value.id,'');
        }
        isLike.value = true;
        ElNotification({
            title: 'Success',
...
const check = ()=>{
    ...
    if(LocalStorageManager.getPrimaryKey('collect',post.value.id)){
...
const collectAction = () =>{
    if(isCollect.value){
        LocalStorageManager.removePrimaryKey('collect',post.value.id);
        let collects = LocalStorageManager.getLocalStorageInfo('collect',userInfo.id.toString()) ?? []
        if(collects.length!==0){
            collects =  collects.filter(v=>{
                return v.id !== post.value.id
            })
        }
        LocalStorageManager.removeLocalStorageInfo('collect',userInfo.id)
        LocalStorageManager.setLocalStorageInfo('collect',{
            posts:[...collects]
        })
        isCollect.value = false;
        ElNotification({
            title: 'Success',
            message: 'Success to cancel collect this Post',
            type: 'success',
            duration:1000
        })

    }else{
        LocalStorageManager.setPrimaryKey('collect',post.value.id);
        LocalStorageManager.setLocalStorageInfo('collect',{
            posts:[{
                ...info.value,
                collect_time:(new Date()).toString()
            }]
        })
        isCollect.value = true
        ElNotification({
            title: 'Success',
            message: 'Success to collect',
            type: 'success',
            duration:1000
        })
    }
...
<template>
  <div v-if="info" @click="goPostDetailPage"  class='post-container' :style="{paddingBottom:( itemProps?.isShowInteract===true || isShowInteract === true )?'20px':'20px'}">
        <div class='post-header '>
            <img  @click="goPersonFilePage"  :src="info.avatar && normalUrl + info.avatar " />
            <div class='post-name'>
        ...
        <div :style="{height:'65%',paddingLeft:( itemProps?.isShowInteract===true || isShowInteract === true )?'60px':'10px',
            ...
            display:'flex',alignItems:'center'}">
                ...
                <div v-if="isShowInteract || itemProps && itemProps.isShowInteract" class='interaction-container' 
                    ...
                    <div @click.stop="collectAction" class='post-share' 
                    :style="{...middleStyle,alignItems:'left',
                    cursor:'pointer',marginRight:0,
                    color:isCollect?'#79bbff':'inherit'
                    }">
                        <el-icon>
                            <CollectionTag/>
                        </el-icon>
